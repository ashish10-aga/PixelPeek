...
export async function validateAnswerAdvanced(
  ...
  try {
    const validationPrompt = `STRICT IMAGE GUESSING GAME VALIDATOR.
...
RULES:
1. Guess MUST describe the SAME MAIN SUBJECT
2. Vague or unrelated answers are REJECTED
3. Only accept if they mean essentially the same thing
4. Be STRICT - reject if there's significant difference
...
{"isValid": boolean, "confidence": 0.0-1.0, "reasoning": ""}`;
    ...
    if (response.ok) {
      ...
      try {
        ...
        // Only accept if Gemini says valid AND confidence is high (0.75+)
        if (result.isValid && (result.confidence || 0) >= 0.75) {
          return {
            isValid: true,
  ...
  // Fallback: ONLY accept if score is very high (strict mode)
  if (fuzzyScore >= 0.88 || jaccardScore >= 0.85) {
    return {
      isValid: true,
