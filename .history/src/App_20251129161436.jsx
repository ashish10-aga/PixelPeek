...
const GameImageContainer = React.lazy(() => import("./components/GameComponents").then(mod => ({ default: mod.GameImageContainer })));
const HintPanel = React.lazy(() => import("./components/GameComponents").then(mod => ({ default: mod.HintPanel })));
...
function GameBoard() {
  ...
  const isCorrect = useCallback((guess, label) => {
  ...
  const handleGuess = useCallback(async (e) => {
    ...
    analyticsService.trackGuessAttempt(state.guess, false);

    if (isCorrect(state.guess, state.label)) {
      ...
      dispatch({ type: GAME_ACTIONS.SET_HINT, payload: "> CORRECT! " + state.label.toUpperCase() });
      ...
      if (newAttempts >= MAX_ATTEMPTS) {
        ...
        dispatch({ type: GAME_ACTIONS.SET_HINT, payload: "> GAME OVER (5 attempts). Answer: " + state.label.toUpperCase() });
      } else {
        // Generate next hint
        dispatch({ type: GAME_ACTIONS.SET_HINT, payload: "> Processing..." });
        const lvl = state.hintLevel + 1;

        try {
          const nextHint = await generateHintFromAI(
            state.desc,
            lvl,
        ...
        {!isMobile && (
          <div className="game-layout-desktop">
            <div className="game-hint-section">
              <Suspense fallback={<div className="hint-panel" aria-hidden="true">LOADING HINT...</div>}>
            ...
            <div className="game-play-section">
              <div className="game-image-wrapper">
                <Suspense fallback={<div className="game-image-container" aria-hidden="true">LOADING IMAGE...</div>}>
                  <GameImageContainer
