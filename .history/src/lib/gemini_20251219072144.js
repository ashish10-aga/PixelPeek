...
export async function generateHintFromAI(description, level = 0, previousHints = [], label = "") {
  try {
    ...
    // Simplified prompt to reduce API complexity and errors
    const prompt = `You are a game hint generator. Give ONE SHORT HINT (max 15 words) at difficulty level ${level} to help guess this: "${safeDescription}". The answer is "${safeLabel}". Output ONLY the hint, nothing else.`;
