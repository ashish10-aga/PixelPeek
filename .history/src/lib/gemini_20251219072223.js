// PixelPeek Gemini Hint Generator - Simplified Version
...
export async function generateHintFromAI(description, level = 0, previousHints = [], label = "") {
  try {
    ...
    const prompt = `Hint for guessing "${desc}" (answer: "${ans}"). Level ${level}. Short (15 words max):`;
    ...
    if (!hint || hint.length < 3) {
      hint = "Look for distinctive details...";
    }
    ...
    const fallbacks = [
      "Study the shapes and colors...",
      "Think about the purpose...",
      "Consider what you'd find it...",
...
export async function generateContextualHint(guess, label, description, level) {
  try {
    ...
    const prompt = `Wrong guess: "${guess}", correct: "${label}". Level ${level} hint (15 words max):`;
    ...
    console.warn("Contextual hint failed:", err.message);
    return null;
  }
}
